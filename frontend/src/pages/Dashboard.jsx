...
function carregarConferenciaSalva() {
  try {
    ...
    if (!Array.isArray(conferencia?.notas)) return null;
...
function Dashboard() {
  ...
  const usuario = useMemo(() => lerUsuarioSalvo(), []);
  const nomeExibicao = obterNomeExibicao(usuario);
  const conferencia = useMemo(() => carregarConferenciaSalva(), []);
  ...
  useEffect(() => {
    ...
    async function carregarResumoProdutos() {
      try {
        ...
        setTotalProdutos(
          Number(resposta.data?.paginacao?.total) ||
            Number(resposta.data?.total) ||
            0
        );
      } catch (error) {
        ...
        if (error.response?.status === 401) {
          setErroProdutos("Sua sessão expirou. Saia e entre novamente.");
        } else {
          setErroProdutos(
        ...
        if (componenteAtivo) setCarregandoProdutos(false);
      }
    }
  ...
  const resumoConferencia = useMemo(() => {
    ...
    const corretos = itens.filter((item) => item.status === "correto").length;
    const erros = itens.filter(
      (item) => item.status !== "correto" && item.status !== "aguardando"
    ).length;
  ...
  return (
    <div className="dashboard">
      <section className="welcome-card">
        <div>
          <span className="welcome-badge">Auditoria de entradas</span>

          <h1>
            {obterSaudacao()}, <strong>{nomeExibicao}</strong>
          </h1>

          <p>
            Importe os relatórios do Host, consulte os produtos cadastrados e
            acompanhe o resultado da conferência atual.
          </p>
        </div>
        ...
        <div className="welcome-actions">
          ...
          <Link to="/conferencia" className="button button-primary">
            <FaPlus />
            Nova conferência
          </Link>
        </div>
      ...
      <section className="stats-grid">
        <article className="stat-card">
          ...
          <div>
            ...
            <small>{resumoConferencia.totalItens} item(ns) analisado(s)</small>
          </div>
        </article>
        ...
        <article className="stat-card">
          ...
          <div>
            ...
            <small>Divergências, sem fator ou não cadastrados</small>
          </div>
        </article>
        ...
        <article className="stat-card">
          ...
          <div>
            ...
            <small>
              {erroProdutos ? erroProdutos : "Total registrado no sistema"}
            </small>
          </div>
        </article>
        ...
        <article className="stat-card">
          ...
          <div>
            ...
            <small>{resumoConferencia.notasCorretas} nota(s) sem erro</small>
          </div>
        </article>
      </section>
      ...
      <section className="content-card">
        ...
        {ultimasConferencias.length === 0 ? (
          <div className="empty-state">
            ...
            <p>Importe um relatório para os dados aparecerem no dashboard.</p>
          </div>
        ) : (
